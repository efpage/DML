...
First, an html-Element can be selected as base element:
    ...
    selectBase(ID)  (ID can be an element ID or en element reference) 
    unselectBase()  -> restor previous base point
...
function extract(s, filter, resultKey) {
  let t = [], k;  // empty array as return
  if (typeof (filter) == "object") {
    if (Object.keys(filter).length > 0) { // Filter was set
      let keys = Object.keys(filter)
      let values = Object.values(filter)
      let doPush = true
      let doArray = false; if (resultKey) doArray = Array.isArray(resultKey); // Check result key
          ...
          if (resultKey) {
            if (doArray) {              // 1: copy multiple keys
              ...
              if (k.includes('"'))
                ...
                o = k       // 2: copy value of key element
            }
          }
          else
            o = item;                   // 3: copy item
      ...
      s.forEach(function (item) {
        if (resultKey)
          t.push(item[resultKey])  // Copy s[resultKey]
        else
...
function extractFit(s, filter, resultKey) {
  return extract(s, fitSelector(filter, s[0]), resultKey)
}
...
function dragElement(elmnt) {
  ...
  function dragMouseDown(e) {
    e = e || window.event;
    if (this != e.target) return  // Leave, if element is child of elmnt
    if (e.offsetY > 25) return              // only top 20 Pixel active

    e.preventDefault();
    let br = elmnt.getBoundingClientRect()
    elmnt.style.left = px(br.left + br.width / 2)   // Change from scaled to absolute positioning
  ...
  function elementDrag(e) {
    ...
    let x = getpx(elmnt.style.left);
    let y = getpx(elmnt.style.top);
    elmnt.style.left = (x - pos1) + "px";
    elmnt.style.top = (y - pos2) + "px";
  }
  ...
  function closeDragElement() {
    ...
    // elmnt.onmousedown = null; // disable drag on exit
    if (onAfterDrag)
      onAfterDrag()
  }
  return this
...
function unselectBase(cnt = 1, oldCnt = -1, msg = "unselectBase") {
  for (let i = 0; i < cnt; i++) {
    if (_baseStack.length == 0) {
      ("DML error: _baseStack empty in popBase()")
      break
    }
    else
...
function nbsp(n = 1) { let s = ""; for (let i = 0; i < n; i++) { s += "\xa0" } print(s) }
...
function image(s, attrib) { let d = make("img", attrib); d.src = s, d.alt = s; return d; }
function span(s, attrib) { return make("span", attrib, s) }
function link(s, link, attrib) { let lnk = make("a", attrib, s); lnk.href = link; return lnk; }
// Create link
...
function expandableTextarea(content, attrib, placeholder) {
  ...
  setAttributes(ta, {
    "rows": "1", "style": /*"border: none; background-color: rgba(0,0,0,0);"+*/
      " box-sizing: border-box; overflow: hidden; min-height: 35px"
  })
...
function inputText(s, options = {}) {
  ...
  let result
  let d = idiv("", options.baseAttrib)
  if (s != "") {
    let lbl = idiv(s, options.labelAttrib)
    if (options.fieldWidth[0])
      lbl.style.width = px(options.fieldWidth[0])
    d.appendChild(lbl)
  }
  result = create("input", options.inputAttrib)
  if (options.fieldWidth[1])
    result.style.width = px(options.fieldWidth[1])
...
------------------------------------------------------*/
function msgBox(s, attrib) {
  let d = inlineDiv(s, attrib)

  d.style.textAlign = "center"
  d.style.position = "fixed"
  d.style.padding = "3px"
  d.style.boxShadow = "5px 5px 6px silver"
  d.style.borderRadius = "8px"
  if (d.style.backgroundColor == "") d.style.backgroundColor = "#ffffff"
  if (d.style.border == '') d.style.border = "thin solid gray"
  if (d.style.left == '') d.style.left = "50%"
  if (d.style.top == '') d.style.top = "50%"
  d.style.transform = "translate(-50%, -50%)"
  if (d.style.width == '')
    d.style.width = "250px"

  selectBase(d)
  br(); button("ok", "width: 150px; height: 24px;"
    + "  display: inline-flex; align-items: center;"
    + " justify-content: center; margin: 8px;").onclick = () => d.remove()
  unselectBase()
}
...
class _canvas2D {
  constructor(attrib) {
    this.canvas = make("canvas", attrib)
  ...
  marker(x, y, d, type = 0, options = {}) {
    let d2
    switch (type) {
      case 0: this.circle(x, y, d, options); break;
      case 1:
        options[center] = true;
  ...
  _marker(x, y, d, type = 0, options = {}) {
    let d2
    switch (type) {
      case 0: this._circle(x, y, d, options); break;
      case 1:
        options[center] = true;
...
function slider(options = {}) {
  ...
  let base = selectBase(idiv("", options.baseattrib))  // Set div as a base, define width and height#
  ...
  if (sl.vertical) {
    ...
    if ((width == 0) || (height == 0)) {
      width = 30; base.style.width = px(width)
      height = 150; base.style.height = px(height)
    }
  }
  else
  // set horizontal slider
  {
    ...
    if ((base.style.width == "") || (base.style.height == "")) {
      width = 150; base.style.width = px(width)
      height = 30; base.style.height = px(height)
    }
    sl.style.top = "-1px";
