...
class _pageWindow {
    constructor(txt, attrib) {
        this.base = selectBase(div("", "width: " + (100 - codefraction) + "%; "))
    ...
    update(txt) {
        ...
        if (sp != DMLgetSP()) {
            ...
            while (DMLgetSP() > sp) unselectBase();
        }
...
class _codeWindow {
    constructor(txt, attrib) {
        this.base = selectBase(div("", "width: " + (100 - codefraction) + "%;" + _bgwhite))  // outer div
        this.error = div("", _bgsilver + "padding-left: 5px;")
    ...
    update(txt) {
        ...
        selectBase(this.innerdiv)  // Open current base
        try {
            let sp = DMLgetSP(); // get stack pointer
...
class _pageEditor {
    constructor(srctxt, baseattr, cwattrib) {
        this.base = selectBase(div("", "display: flex"))
